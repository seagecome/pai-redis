package com.pai.base.concurrent;
...
public class ForkJoinPool<T> {
	 ...
	 *
	 * @param tasks 待处理记录数目
	 * @param sizePerTask 每个任务处理记录的数目
	 * @return
	 */
	public static <T> List<List<T>> splitTaskBySizePerTask(List<T> tasks, Integer sizePerTask) {
		List<List<T>> list = Lists.newArrayList();
		if(tasks==null || sizePerTask <= 0){
			return list;
		}
		if(tasks.size() < sizePerTask){
			list.add(tasks);
			return list;
		}


		int baseNum = tasks.size() / sizePerTask; // 每个list的最小size
		int remNum = tasks.size() % sizePerTask; // 得到余数

		int index = 0;
		for (int i = 0; i < baseNum; i++) {

			List<T> ls = Lists.newArrayList();
			for (int j = index; j < sizePerTask + index; j++) {
				ls.add(tasks.get(j));
			}
			list.add(ls);
			index += sizePerTask;
		...
		if(remNum > 0){
			...
			for(int j=0;j<remNum;j++){
				ls.add(tasks.get(baseNum*sizePerTask+j));
			}
			list.add(ls);
		}
		return list;
